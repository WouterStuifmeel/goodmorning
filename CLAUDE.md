...
Real providers now built for all three swappable stages: `RssNewsProvider` (`config/feeds.json`, currently 4 direct publisher feeds - NOS buitenland/algemeen for world/Dutch, RTV Utrecht for local, TechCrunch for technology; switched from Google News aggregator feeds because their `<link>` is an opaque redirect-tracking URL that resolves to a Google consent interstitial rather than the article - see git history around 2026-09-20 - and because the aggregator entries carry no real per-story summary text for the script model to ground on, which invited hallucination), `OpenAIScriptProvider` (structured-output script generation, grounding validated post-hoc against supplied news candidates, bounded retry on incomplete output), and `OpenAITTSProvider` (`gpt-4o-mini-tts`, streamed WAV - see `app/providers/openai/`). The audio renderer normalizes every section's sample rate/channels before concatenation, since OpenAI's TTS output (24kHz mono) differs from the mock's (44.1kHz mono) - see `FFmpegAudioRenderer._normalize` in `app/audio/renderer.py`.
...
**The KNMI bulletin is nationwide, not local** - it covers the whole of the Netherlands and is often broken down by named sub-region (e.g. "noorden"/north = Groningen, Drenthe, Friesland; "midden"/central; "zuiden"/south; coastal vs inland). A first live test surfaced exactly this: the script narrated "the northern area" conditions to a listener in Utrecht. Fixed via prompt, not parsing - `OpenAIScriptProvider`'s bulletin instructions (`app/providers/openai/script.py`'s `_user_prompt`) now spell out the sub-region names and tell the model to use only the nationwide-general parts plus whichever sub-region actually covers the listener's own location, and `_SYSTEM_PROMPT` has a matching hard rule. Regex/structural parsing of the bulletin was considered and rejected - the free-text format isn't guaranteed stable enough to parse reliably, and prompt-based filtering has tested reliable so far. Worth re-checking if regional bleed-through recurs.
